...
function ListSubject({userId}) {
    ...
    React.useEffect(() => {
        fetchData();
            // axios.post('/api/subjects', userId)
            //     .then((res) => {
            //         const subjectsFromServer = res.data;
            //         const updatedList = new Subjects([...list.subjects]);
            //         subjectsFromServer.forEach((subject, subjectIndex) => {
            //             updatedList.addSubject(subject.name);
            ...
            //             })
            //                 .then((res)=>{
            //                     const chaptersFromServer = res.data;
            //                     chaptersFromServer.forEach((chapter, chapterIndex)=>{
            //                         updatedList.addChapter(subjectIndex, chapter.chapterName);
            //                         updatedList.subjects[subjectIndex].chapters[chapterIndex].checked = chapter.checked;
            //                         updatedList.setChapterProgressPercent(subjectIndex, chapterIndex);
            //                     })
    ...
    return (
        <div>
            ...
            <AreaContainer>
                <OverviewArea list={list} changeList={changeList} changeShowingIndex={onChangeShowingIndex} userId={userId}/>
                {showingIndex !== -1 ?
